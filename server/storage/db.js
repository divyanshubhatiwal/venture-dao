...
/**
 * These four used to be raw SQL inside kyc.js, which meant the storage layer
 * had a second entrance. Moving them here is what kept this migration to one
 * file plus its callers.
 */
...
export async function upsertKycRecord({
  ...
  await collections.kycRecords().replaceOne(
    { _id: userId },
    {
      status,
      method,
      ...
      panSealed,
      panLast4,
      addressSealed: addressSealed ?? null,
      submittedAt,
      reviewedAt: null,
      reviewedBy: null,
      reason: null,
  ...
  return findKycRecord(userId)
}

export const findKycRecord = async (userId) => clean(await collections.kycRecords().findOne({ _id: userId }), 'userId')

export const setKycLiveness = (userId, { livenessAt, livenessNote }) =>
  collections.kycRecords().updateOne({ _id: userId }, { $set: { livenessAt, livenessNote: livenessNote ?? null } })

export const setKycReview = (userId, { status, reviewedAt, reviewedBy, reason }) =>
  collections
    .kycRecords()
    .updateOne({ _id: userId }, { $set: { status, reviewedAt, reviewedBy, reason: reason ?? null } })
