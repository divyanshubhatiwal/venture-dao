...
import { decide } from '../lib/agent/decision.js'
...
export function botStatus(accountId = 'default') {
  ...
  const startingEquity = ledger.startingBalance || config.startingBalance
  const currentBalance = ledger.balance || startingEquity
  const positions = ledger.positions ?? []
  const trades = ledger.trades ?? []
  const tradesToday = ledger.tradesToday ?? 0
  const unrealisedPnl = ledger.unrealisedPnl ?? 0
  ...
  return {
    ...
    progress: dailyProgress({
      startingEquity,
      realisedPnl: currentBalance - startingEquity,
      unrealisedPnl,
      config,
    }),
    report: dailyReport({
      startingEquity,
      endingEquity: currentBalance,
      trades,
      config,
      markets: trades.map((t) => t.symbol),
    }),
